angular.module('theme.atencionMedicaAmb', ['theme.core.services','ui.grid.edit','luegg.directives'])
  .controller('atencionMedicaAmbController', ['$scope', '$route', '$filter', '$sce', '$interval', '$location', '$anchorScroll','$modal','$uibModal', '$bootbox', '$window', '$http', '$theme', '$log', '$timeout', 'uiGridConstants', 'pinesNotifications', 'hotkeys', 
    ...
    'odontogramaServices',
    function($scope, $route, $filter, $sce, $interval, $location, $anchorScroll, $modal, $uibModal, $bootbox, $window, $http, $theme, $log, $timeout, uiGridConstants, pinesNotifications, hotkeys, 
      atencionMedicaAmbServices,
      diagnosticoServices,
      solicitudProcedimientoServices,
      afeccionServices,
      ...
      $scope.btnConsultarPacientesAtencion = function () { 
        ...
        if( validateButton ){ 
          ...
          atencionMedicaAmbServices.sListarPacientesSinAtender($scope.fBusqueda).then(function (rpta) { 
            ...
            //     $scope.gridApi.selection.selectRow($scope.gridOptionsPPA.data[0]);
            //   }
            // });
          });
        }else{
    ...
    $scope.btnAtenderAlPaciente = function (estadoAtendido, mySelectionAtencionGrid) { 
      ...
      if(pEstadoAtendido === false){ // EN REGISTRAR, VALIDAR SI ES ATENCION DEL DIA O NO 
        if($scope.mySelectionGrid[0].situacion.autorizado === 2){ // NO AUTORIZADO 
          pinesNotifications.notify({ title: 'Bloqueo de Atención', text: 'Esta atención ha sido bloqueada, porque no es del día actual.', type: 'danger', delay: 3000 }); 
          return false;
        }
        var mySelectionAtencionGrid = $scope.mySelectionGrid;
      }
      // console.log(mySelectionAtencionGrid);
      if( mySelectionAtencionGrid[0].idtipoproducto == 12 ){ // CONSULTA MÉDICA 
        ...
        if(pEstadoAtendido && pEstadoAtendido === 'si'){ 
          ...
          atencionMedicaAmbServices.sListarDiagnosticosDeAtencion(arrParams).then(function (rpta) { 
            ...
            $scope.gridOptionsDiagnostico.data = rpta.datos; 
            // console.log($scope.gridOptionsDiagnostico.data); 
          });
        }else{ 
        ...
        /*=================================================*/
        /*  AFECCIONES MEDICAS made by ALEX                */
        /*=================================================*/
